...
declare global {
  interface Window {
    ...
    userengage?: (eventName: string, payload?: Record<string, string | number | keyof typeof CurrencyCode | undefined>) => void;
  }
}
...
export default class UsercomBrowserDriver implements AnalyticsDriver {
  ...
  protected async trackCustom(event: CustomAnalyticsEvent<unknown>): Promise<void> {
    ...
    return this.push(data.event_name, data.event_payload as Record<string, string | number | undefined | keyof typeof CurrencyCode>);
  }

  protected push(eventName: string, payload?: Record<string, string | number | keyof typeof CurrencyCode | undefined>): void {
  ...
  protected checkIfUsercomLoaded(timeout = 0): Promise<void> {
    return new Promise((resolve, reject) => {
      if (window.userengage && window.UE) {
        ...
        setTimeout(() => {
          this.checkIfUsercomLoaded(timeout + UsercomBrowserDriver.AVAILABILITY_CHECK_TIMEOUT).then(() => resolve()).catch(() => reject());
